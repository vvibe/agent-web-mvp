...
func runWindowsTaskInstall(args []string) {
	...
	// equivalent of "run only when logged on, no elevation". Hidden
	// keeps the foreground console invisible — without it, `vvibe run`
	// pops a PowerShell-style window on every logon, which is hostile.
	ps := strings.Join([]string{
		fmt.Sprintf(`$action = New-ScheduledTaskAction -Execute "%s" -Argument "run" -WorkingDirectory "%s"`, exe, filepathDirOf(exe)),
		`$trigger = New-ScheduledTaskTrigger -AtLogOn -User $env:USERNAME`,
		`$principal = New-ScheduledTaskPrincipal -UserId $env:USERNAME -LogonType Interactive -RunLevel Limited`,
		// Hidden so the daemon's console window doesn't pop up on logon.
		// StartWhenAvailable + RestartCount let it recover from transient
		// failures (laptop suspended at logon, etc.).
